...
import { ArrowLeft, Cpu, Activity, MapPin, Zap, RefreshCw, Server, CheckCircle2, XCircle } from 'lucide-react';

export default function DevicePage() {
  const navigate = useNavigate();
  const { sensorData, deviceData, isDemoMode, setIsDemoMode, bleStatus, firebaseConnected, isConnected } = useApp();
  ...
  // IMU values
  const imu = deviceData?.imu;
  const imuLabel = imu?.x != null
    ? `X: ${imu.x}  Y: ${imu.y}  Z: ${imu.z}`
    : 'No data';

  // Altitude
  const altLabel = deviceData?.altitude != null ? `${deviceData.altitude} m` : '--';
    ...
    <div className="page-container" style={{ paddingBottom: '90px' }}>
      ...
      <div className="glass-panel mb-6 animate-in stagger-2" style={{ position: 'relative', overflow: 'hidden', padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          {/* IMU */}
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'flex-start', gap: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: connected ? 'rgba(16, 185, 129, 0.1)' : 'rgba(244, 63, 94, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div>
              ...
              {connected && (
                <p className="text-xs font-mono mt-1" style={{ color: 'var(--accent-teal)', lineHeight: 1.7 }}>
                  {imuLabel}
                </p>
              )}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div>
              ...
              <p className="text-xs text-muted">{connected ? `${deviceData?.bpm || sensorData?.heartRate || 0} BPM` : 'Offline'}</p>
            </div>
          </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'flex-start', gap: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div>
              ...
              {connected && (
                <p className="text-xs font-mono mt-1" style={{ color: 'var(--accent-teal)' }}>
                  Alt: {altLabel}
                </p>
              )}
            </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div>
              ...
              <p className="text-xs text-[var(--accent-teal)] font-bold">{connected ? `${deviceData?.battery ?? sensorData?.battery ?? 0}%` : '--'}</p>
            </div>
          </div>
        </div>
      </div>
